    ...
    return (
        <main className="grid place-items-center h-full bg-inherit">
            <div className="flex flex-col gap-5 items-center">
                <TypeWriter text="Kryp.Dev" duration={1} onWritingEnd={onWritingEnd} className="text-8xl select-none" />
                <div ref={linksRef} className="flex gap-8 justify-evenly max-h-0 h-auto opacity-0 duration-[2s] transition-all [animation-delay:2s] animate-appear">
                    {links.map((link, index) => (
                        <div key={index} className="border-2 p-3 rounded-xl cursor-pointer hover:animate-shake">
                            <a href={link.url} target="_blank">
                                <link.iconComponent className="size-12" />
                            </a>
                        </div>
                    ))}
                </div>
